...
def bfs():
    ...
    while Q:
        ...
        for ddy ,ddx in zip(dy,dx):
            ny,nx = ddy+y,ddx+x 
            if 0<=ny<N and 0<=nx<M: # 한번도 방문한적이 없는 경우 
                # 다음 위치가 벽이 아니어서 그냥 이동가능한 경우
                if maps[ny][nx] ==0 and visited[ny][nx][wall]==0:
                    Q.append((ny,nx,wall))
                    visited[ny][nx][wall] = visited[y][x][wall]+1
                # 다음위치가 벽인 경우 , 벽을 부순 횟수가 K보다 작은 경우
                elif maps[ny][nx] == 1 and wall <K and visited[ny][nx][wall+1] == 0:
